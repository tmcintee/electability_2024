...
polls_filtered <- polls_processed %>%
  ...
  ungroup()
other_dems <- c("Bernard Sanders","Gavin Newsom","Hillary Clinton","Kamala Harris","Pete Buttigieg")
...
polls_summarized <- polls_filtered %>%
  ...
  mutate(Net = Mean.Republican.Vote.Bonus - Mean.Democratic.Vote.Bonus)
